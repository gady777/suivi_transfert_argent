  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Tranfert par tranche : envoi</h1>
          </div>
          <div class="col-sm-6">
      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <div class="col">
            ...
            href="{{route('admin.transfer.tranche.instance.new.envoi',['id'=>$t->id])}}">
            ...
            <p> 
              <br>
                Transfert par tranche de <strong>{{number_format($t->amount,2,'.',' ')}} 
                  {{$t->devise()->abbreviation}}</strong> - 
                  <strong>{{$t->user()->pseudo}} ({{$t->user()->email}})</strong>, <b>{{$t->amount_cfa}} F CFA</b> .
            </p>
          </div>
        </div> <br>
          ...
          <div class="col">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title"> 
                  <span class="font-weight-bolder">ENVOI</span> <br>
                  Reste: <strong>{{number_format($t->solde_envoi,2,'.',' ')}} F CFA</strong>
                </h3>
              </div>
              <div class="card-body">
                  ...
                  <tbody>
                    @forelse($rs as $p)
                      <tr>
                        ...
                        <td>{{date('d-m-Y H:i',strtotime($p->pay_date))}}</td>
                        <td>{{$p->recipient() ? ($p->recipient()->name.' - '.$p->recipient()->email):''}}</td>
                        <td>{{$p->method()->name}}</td>
                        <td>{{number_format($p->amount,2,'.',' ')}} {{$p->devise()->abbreviation}} </td>
                        <td>{{number_format($p->solde_envoi,2,'.',' ')}} {{$p->devise()->abbreviation}}</td>
                        {{--<td>
